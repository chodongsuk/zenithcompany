package kr.ds.config;

import android.content.Context;

public class Config {
	public Context mContext;
	
	public static String URL = "http://zenithcompany1.cafe24.com/json/";
	public static String URL_LIST = "list_v2.php";
	public static String URL_CODE = "code.php";
	public static String URL_WEB = "web.php";
	public static String URL_PRODUCT = "product.php";
	public static String URL_REPORTER = "reporter.php";
	public static String URL_POPUP = "popup.php";
	public static String URL_CODE_PRODUCT = "code_product.php";
	public static String URL_CODE_REPORTER = "code_reporter.php";
	//gcm
	public static final String ANDROID_ID = "android_id";
	public static final String SENT_TOKEN_TO_SERVER = "sentTokenToServer";
	public static final String REGISTRATION_COMPLETE = "registrationComplete";
	public static final String TOKEN = "token";
	//gcmend
	public static final String CODE_DATE = "2017041202";
	public static final String CODES = "codes";
	public static final String ALL_CODE = "all_code";
}
